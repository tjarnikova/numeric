...
def first_time_step(u, h, v, g, H, dt, dx, ho, gu, gh, n_grid):
    ...
    factor = -gu * ho 
    midpoint = n_grid / 2
    u.now[midpoint - .5] = factor
    u.now[midpoint + .5] = -factor
    h.now[:] = 0
    h.now[midpoint] = ho - g * H * ho * dt ** 2 / (1 * dx ** 2)
    v.now[:] = 0 


def leap_frog(u, h, v, f, gu, gh, n_grid, dt):
    ...
    for pt in np.arange(1, n_grid - 1):
        u.next[pt] = u.prev[pt] - gu * (h.now[pt + 1] - h.now[pt])*2+f*v.now[pt]*2*dt
        h.next[pt] = h.prev[pt] - gh * (u.now[pt] - u.now[pt - 1])*2
...
def make_graph(u, h, v, dt, n_time):
    ...
    fig, (ax_u, ax_h, ax_v) = plt.subplots(3,1, figsize=(10,10))
    ...
    for time in range(0, n_time, interval):
        ...
        u_X = np.arange(0,len(u.store[:,time]))
        ax_u.plot(u_X,(u.store[:,time]), color = colorVal)
        v_X = np.arange(0,len(v.store[:,time]))
        h_X = np.arange(-.5,len(h.store[:,time])-0.5)
        ax_v.plot(v_X,v.store[:, time], color=colorVal)
        ax_h.plot(h_X,h.store[:, time], color=colorVal)
        
        ax_u.set_xlim([0,len(u.store[:,time])-1])
        ax_h.set_xlim([0,len(h.store[:,time])-1])
        ax_v.set_xlim([0,len(v.store[:,time])-1])
...
#     n_time, n_grid = map(int, args)
    ...
    first_time_step(u, h, v, g, H, dt, dx, ho, gu, gh, n_grid)
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, h, v, f, gu, gh, n_grid, dt)
        #leap_frog(u, h, gu, gh, n_grid)
        boundary_conditions(u.next, h.next, v.next, n_grid)
