...
def ice_load():
	filelist = 'icec.day.mean.2014.v2.nc'
...
def cloud_load():
	filelist = 'tcdc.eatm.gauss.2014.nc'
	with Dataset(filelist) as nc:
		#print(nc.variables)
		tcdc=nc.variables['tcdc'][...]
		lat =nc.variables['lat'][...] 
		lon = nc.variables['lon'][...]
		time = nc.variables['time'][...]
	...
	#dealing with the mask in tcdc - assuming it's a value of 0
	# le_mask = np.ma.getmask(tcdc)
	# print(le_mask)
	# #tcdc[le_mask]=0
	...
	Cloud_cover = collections.OrderedDict()
	Cloud_cover['Jan'] = np.mean(tcdc[Jan,latstart:latend,:])
	Cloud_cover['Feb'] = np.mean(tcdc[Feb,latstart:latend,:])
	Cloud_cover['Mar'] = np.mean(tcdc[Mar,latstart:latend,:])
	Cloud_cover['April'] = np.mean(tcdc[April,latstart:latend,:])
	Cloud_cover['May'] = np.mean(tcdc[May,latstart:latend,:])
	Cloud_cover['June'] = np.mean(tcdc[June,latstart:latend,:])
	Cloud_cover['July'] = np.mean(tcdc[July,latstart:latend,:])
	Cloud_cover['Aug'] = np.mean(tcdc[Aug,latstart:latend,:])
	Cloud_cover['Sep'] = np.mean(tcdc[Sep,latstart:latend,:])
	Cloud_cover['Oct'] = np.mean(tcdc[Oct,latstart:latend,:])
	Cloud_cover['Nov'] = np.mean(tcdc[Nov,latstart:latend,:])
	Cloud_cover['Dec'] = np.mean(tcdc[Dec,latstart:latend,:])
	...
	for i in range(0,12):
		if i == 0:
			cloudvals[i] = dataMap.get('Jan')
		elif i ==1:
			cloudvals[i] = dataMap.get('Feb')
		elif i ==2:
			cloudvals[i] = dataMap.get('Mar')
		elif i ==3:
			cloudvals[i] = dataMap.get('April')
		elif i ==4:
			cloudvals[i] = dataMap.get('May')
		elif i ==5:
			cloudvals[i] = dataMap.get('June')
		elif i ==6:
			cloudvals[i] = dataMap.get('July')
		elif i ==7:
			cloudvals[i] = dataMap.get('Aug')
		elif i ==8:
			cloudvals[i] = dataMap.get('Sep')
		elif i ==9:
			cloudvals[i] = dataMap.get('Oct')
		elif i ==10:
			cloudvals[i] = dataMap.get('Nov')
		elif i ==11:
			cloudvals[i] = dataMap.get('Dec')


	days_tointerp = np.zeros([14])
	cloud_tointerp = np.zeros([14])
	cloud_tointerp[0] = (cloudvals[0]+cloudvals[11])/2
	cloud_tointerp[13] = (cloudvals[0]+cloudvals[11])/2
	days_tointerp[0] = 1
	days_tointerp[13] = 365
	for i in range(1,13):
		days_tointerp[i] = mids[i-1]
		cloud_tointerp[i] = cloudvals[i-1]
	...
	cloud_interp = ynew/100

	return(cloud_interp)

def wind_load():
	wind_file = 'wspd.sig995.mon.ltm.nc'

	with Dataset(wind_file) as nc:
	#print(nc.variables)
...
#these values are taken from WOA 2009 (as a mat file) 
...
def met_get():
	...
	met_var['wind'] = wind_load()
	met_var['par'] = par_load()
	met_var['ccsen'] = ccsen_load()
	met_var['cloud'] = cloud_load()
