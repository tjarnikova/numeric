...
class Integrator:
    ...
    def met_forcing(self, timeStep, met):
        ...
        cloud = met.get('cloud')
        cloud = cloud[int(timeStep)]
        met_force = {'ice': ice, 'wind': wind, 'sst': sst, 'par': par, 'icemelt': icemelt, 'ccsen': ccsen, 'cloud' : cloud}# 'par': par}
...
#       pdb.set_trace()
    ...
    def timeloop5fixed(self, met):
        ...
        for theTime in timeSteps[:-1]:
            ...
            cloudvals.append(cloud)


        yvals = np.array(yvals).squeeze()
        errorVals = np.array(errorList).squeeze()
        met_dict = {'ice': icevals, 'wind': windvals, 'sst': sstvals, 'par': parvals, 'icemelt' : icemeltvals, 'ccsen' : ccsenvals, 'cloud': cloudvals}
        #metvals 
